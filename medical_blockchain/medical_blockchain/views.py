...
def uploadfile(request):
    ...
    #         encrypted_data = fernet.encrypt(csv_data)
            ...
            # f1=open('encrypted_file.bin','r')    
            # f1='encrypted_file.bin'    

            # Save the encrypted data to the database
    #         encrypted_csv = EncryptedCSV.objects.create(encrypted_data=encrypted_data)
    #         encrypted_csv.save()
...
def blk(request):
    ...
    #             f.write(encrypted_bytes3)   
            ...
            ftp.login(user='aniketrajani03@gmail.com', passwd='cloud03**')  
            
            ftp.cwd('/ML_BLOCK1')
            file = open(r"E:\Work\Final_Year_Project\medical_blockchain\encrypted_file1.bin", 'rb')  # replace 'path/to/local/file' with the path to your local file
            ftp.storbinary('STOR encrypted_file1.bin', file)  # replace 'file.txt' with the desired filename on DriveHQ      
             
            ftp.cwd('/ML_BLOCK2')
            file = open(r"E:\Work\Final_Year_Project\medical_blockchain\encrypted_file2.bin", 'rb')  # replace 'path/to/local/file' with the path to your local file
            ftp.storbinary('STOR encrypted_file2.bin', file)  # replace 'file.txt' with the desired filename on DriveHQ                
                                
            ftp.cwd('/ML_BLOCK3')
            file = open(r"E:\Work\Final_Year_Project\medical_blockchain\encrypted_file3.bin", 'rb')  # replace 'path/to/local/file' with the path to your local file
...
def down(request):
    ...
    filename = 'encrypted_file1.bin'
    local_filepath = r'C:\Users\anike\Downloads\{}'.format(filename)

    # download the file from DriveHQ to a local file
    ...
    # Create a Fernet instance with the key
    key =  Fernet.generate_key()# replace with the actual key
    fernet = Fernet(key)
...
def health(request):
    ...
    replace_row_in_csv("E:\Work\Final_Year_Project\medical_blockchain\patientdata.csv",nlis)
